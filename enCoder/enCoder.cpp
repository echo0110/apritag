...
#if 0
        memset(&awp, 0, sizeof(AdvanceWorkPara));
        awp.num_frames = 0;
        /* gop:I帧间隔 */
        awp.gop_mode = 0;
        awp.gop_len = wp.out_fps;
        awp.vi_len = 0;
        /* fps_out:输出帧率 */
        awp.fps_out_flex = 0;   // (0:[固定的输出帧率, 输出帧率=(fps_out_num/fps_out_den)], 1:[可变的输出帧率,完成帧编码后马上输出])
        awp.fps_out_num = wp.out_fps;    // (0:[使用默认值], n:[使用n值]) -- 默认值为30
        awp.fps_out_den = 1;    // (0:[使用默认值], n:[使用n值]) -- 默认值为1
        /* fps_in :输入帧率 */
        awp.fps_in_flex = 0;    // (0:[固定的输入帧率, 输入帧率=(fps_in_num/fps_in_den)], 1:[可变的输入帧率])
        awp.fps_in_num = wp.out_fps;     // (0:[使用默认值], n:[使用n值]) -- 默认值为30
        awp.fps_in_den = 1;     // (0:[使用默认值], n:[使用n值]) -- 默认值为1
        /* bps:码率 */
        awp.rc_mode = (int32_t)MPP_ENC_RC_MODE_VBR;
        awp.bps_target =wp.width * wp.height / 8 * (awp.fps_out_num / awp.fps_out_den);
        awp.bps_max = awp.bps_target;        // 码率上限(0:[根据bps_target自动计算值], n:[使用n值])
        awp.bps_min = 0;        // 码率下限(0:[根据bps_target自动计算值], n:[使用n值])
    	ret = set_encMedia_channel_workPara(pPara->videoChn_Id, &wp, &awp);
#else
